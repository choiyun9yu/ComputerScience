...
### 1-1. 삽입 정렬 최선일 경우 
삽입 정렬이 최선일 경우 데이터 이동 횟수가 0회이고, 데이터 비교 횟수는 $n - 1$회 이다. 최초 축이 되는 자신을 제외한 데이터와 1번씩만 비교하기 때문이다.
...
### 1-2. 삽입 정렬 최악인 경우
삽입 정렬이 최악일 경우는 데이터가 역으로 정렬되어 있을 때이다. 이때 데이터 이동 횟수는 $\frac{1}{2}(n + 1)n - 1$이고, 데이터 비교 횟수는 $\frac{1}{2}n(n - 1)$이다.

데이터의 비교 횟수부터 살펴보자. 데이터 비교는 한 번 정렬할 때마다, 정렬 완료된 데이터 바로 옆에 위치한 데이터가 정렬 완료된 데이터 모두와 비교한다. 최초에는 정렬완료된 데이터가 1개 뿐이라서 1회만 비교해도 되지만 마지막에는 n-1개의 데이터와 비교해야한다.
...
### 4-1. 퀵 정렬이 최악일 경우 데이터 비교 횟수 $O(n^2)$
...
### 4-2. 퀵 정렬이 최상일 경우 데이터 비교 횟수 $O(n \log n)$
최상의 경우는 매 정렬 시마다 데이터를 약절만으로 나눌 수 있는 값을 피벗으로 삼는 것이다. 이때 정렬 횟수는 데이터가 1개 이하일 때를 제외한 $\log_2n - 1$표현할 수 있다. 

$\log_2$는 &n&을 몇 번 나누면 $1$이 되는지 나타내므로 
